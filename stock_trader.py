...
class StockTrader:
    ...
    def show_confirmation_dialog(self, action, price):
        """显示交易确认对话框"""
        ...
        dialog.title(f'{"买入" if action == "buy" else "卖出"}确认')
        dialog.geometry('300x150')
        dialog.transient(self.root)

        message = f'确认{"买入" if action == "buy" else "卖出"}\n当前价格: {price}'
        ...
        tk.Button(dialog, text='确认', command=on_confirm).pack(side=tk.LEFT, padx=20, pady=10)
        tk.Button(dialog, text='取消', command=on_cancel).pack(side=tk.RIGHT, padx=20, pady=10)
    ...
    def buy(self, price):
        """执行买入操作"""
        if self.capital >= price and self.show_confirmation_dialog('buy', price):
            self.capital -= price
            self.holdings += 1
    ...
    def sell(self, price):
        """执行卖出操作"""
        if self.holdings > 0 and self.show_confirmation_dialog('sell', price):
            self.capital += price
            self.holdings -= 1
            trade = {
                'type': 'sell',
